...
import { buildToolArguments, extractGlobalOptions, getFlag, hasFlag, parseFlags, } from "./args.js";
import { assertHasAtLeastOneKey, clearStoredProfile, getProductDisplay, loadConfig, resolveConfigPath, resolveRuntimeConfig, saveConfig, setStoredKey, unsetStoredKey, } from "./config.js";
import { authHelp, callHelp, commandHelp, commandsHelp, mainHelp, productsHelp } from "./help.js";
import { commandInputSchema, commands, commandsForProduct, findCommand, toKebabCase, validateCommandArgs, } from "./registry.js";
import { writeData, writeError, writeText } from "./output.js";
import { CLI_VERSION, DEFAULT_PROFILE, PRODUCTS, isProductId, redactSecret, } from "./types.js";
export async function run(argv, context = {}) {
    ...
    try {
        ...
        if (command === "auth" || command === "login")
            return await handleAuth(command === "login" ? ["set", ...rest] : rest, handlerContext);
        if (command === "logout")
            return await handleAuth(["unset", ...rest], handlerContext);
        if (command === "status")
            return await handleAuth(["list", ...rest], handlerContext);
        if (command === "commands" || command === "tools")
            return await handleCommands(rest, handlerContext);
        if (command === "products")
            return await handleProducts(handlerContext);
        if (command === "mcp")
            return await handleMcp(handlerContext);
        if (command === "call" || command === "run")
            return await handleCall(rest, handlerContext);
...
async function handleAuth(tokens, context) {
    ...
    if (action === "set") {
        ...
        if (!productId || !isProductId(productId))
            throw usageError("Usage: forge auth set <product> --key-stdin");
        ...
        writeData(context.stdout, { profile, product: productId, displayName: getProductDisplay(productId), configured: true, configPath }, context.globals);
        return 0;
    }
    if (action === "unset") {
        const [productId] = rest;
        if (!productId || !isProductId(productId))
            throw usageError("Usage: forge auth unset <product>");
...
async function handleProducts(context) {
    const runtime = await resolveRuntimeConfig(context.globals, context.env);
    writeData(context.stdout, PRODUCTS.map((product) => ({
        product: product.id,
        displayName: product.displayName,
...
async function handleCommands(tokens, context) {
    const [action, ...rest] = tokens;
    if (!action || action === "help" || action === "--help") {
    ...
    if (action === "list") {
        ...
        if (productFilter && !isProductId(productFilter))
            throw usageError(`Unknown product "${productFilter}"`);
        ...
        const selected = (productFilter ? commandsForProduct(productFilter) : [...commands])
        ...
        writeData(context.stdout, selected.map((command) => ({
            name: command.name,
    ...
    if (action === "describe") {
        const [name] = rest;
        if (!name)
            throw usageError("Usage: forge commands describe <command-name>");
        const match = findCommand(name);
        if (!match)
            throw unknownCommand(name);
        writeData(context.stdout, {
            ...match.command,
...
async function handleCall(tokens, context) {
...
async function handleDirectCommand(tokens, context) {
    const match = findCommand(tokens);
    if (!match)
        throw unknownCommand(tokens.join(" "));
    return executeCommand(match.command, tokens.slice(match.consumed), context);
}
async function executeCommand(command, argTokens, context) {
    const runtime = await resolveRuntimeConfig(context.globals, context.env);
...
function createExecutor(runtime, context) {
...
async function resolveKeyInput(flags, stdin) {
    ...
    if (hasFlag(flags, "key-stdin")) {
        ...
        if (!value.trim())
            throw authError("No API key was provided on stdin");
        return value.trim();
    }
    ...
    if (keyFile) {
        ...
        if (!value.trim())
            throw authError(`No API key found in ${keyFile}`);
        return value.trim();
    }
...
function helpFor(topic) {
    if (topic === "auth" || topic === "login")
        return authHelp();
    if (topic === "logout" || topic === "status" || topic === "products")
        return topic === "products" ? productsHelp() : authHelp();
    if (topic === "commands" || topic === "tools")
        return commandsHelp();
...
export async function writeDefaultConfig(configPath) {

...
const exportAliases = { sf: "salesforge", wf: "warmforge", inst: "instantly", sl: "smartlead", reply_io: "replyio" };
/** Reject formerly advertised inputs which the public handlers do not consume. */
function unsupported(args, ctx, keys) {
    for (const key of keys)
        if (args[key] !== undefined)
            ctx.addIssue({ code: "custom", path: [key], message: "Not supported by this public API; use the documented filters" });
}
/** Require exactly one of the public enrichment identity representations. */
function enrichmentIdentity(args, ctx) {
    ...
    if (ids === people)
        ctx.addIssue({ code: "custom", message: "Provide exactly one of personIDs or people" });
}
/** Require one identity for synchronous enrichment without silently combining people. */
function syncIdentity(args, ctx) {
    ...
    if (identities.filter(Boolean).length !== 1)
        ctx.addIssue({ code: "custom", message: "Provide exactly one identity: personID, linkedinURL, or firstName + lastName + companyDomain/company" });
}
export const suiteOptions = {
    ...
    primeforge_export_mailboxes: {
        ...
        refine: (args, ctx) => {
            if (!platforms.safeParse(args.platform).success)
                ctx.addIssue({ code: "custom", path: ["platform"], message: "Provide a supported export platform" });
            if (args.exportType !== undefined && (exportAliases[String(args.exportType)] ?? args.exportType) !== args.platform)
                ctx.addIssue({ code: "custom", message: "platform and exportType must agree" });
...
//# sourceMappingURL=suite-options.js.map
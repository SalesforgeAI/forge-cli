...
export const endpointOptions: Record<string, EndpointOptions> = {
  list_contacts: {
  ...
  bulk_create_contacts: { description: "Create or update 1–100 contacts matched by email or LinkedIn URL. Each entry requires tags or tagIds.", refine: (a, ctx) => {
    if (!Array.isArray(a.contacts) || !a.contacts.length) ctx.addIssue({ code: "custom", path: ["contacts"], message: "Provide 1–100 contacts" });
  } },
  list_mailboxes: {
  ...
  update_mailbox_connection_settings: {
    ...
    refine: (a, ctx) => {
      if (a.smtp === undefined && a.imap === undefined) ctx.addIssue({ code: "custom", message: "Provide smtp or imap settings" });
    },
  },
  ...
  list_sequences: {
    ...
    pageOffset: true,
    refine: (a, ctx) => {
      if (a.offset !== undefined && a.page !== undefined) ctx.addIssue({ code: "custom", message: "Use page or offset, not both" });
      if (a.offset !== undefined && Number(a.offset) % Number(a.limit ?? 20) !== 0) ctx.addIssue({ code: "custom", path: ["offset"], message: "Must be a multiple of limit; use page for multichannel pagination" });
    },
  },
  ...
  preflight_enrollments: {
    ...
    refine: (a, ctx) => {
      if (a.limit === undefined && (!a.filters || !Object.keys(a.filters).length)) ctx.addIssue({ code: "custom", message: "Provide at least one filter or a limit" });
    },
  },
  ...
  confirm_enrollment_preflight: {
    ...
    refine: (a, ctx) => {
      if (a.action === "move" && a.skipReplied === undefined) ctx.addIssue({ code: "custom", path: ["skipReplied"], message: "Required when action is move" });
    },
  },
  start_email_validation: {
    ...
    refine: (a, ctx) => {
      if (!a.filters || !Object.keys(a.filters).length) ctx.addIssue({ code: "custom", path: ["filters"], message: "Provide non-empty contact filters" });
    },
  },

import { CLI_VERSION, PRODUCTS, type CommandDefinition } from "./types.js";
...
export function callHelp(): string {
  return `Usage:
...
export function productsHelp(): string {
  return `Usage:
  forge products
  forge status
...
function optionUsage(params: readonly NonNullable<CommandDefinition["params"]>[number][]): string {
  const rendered = params.map((input) => input.required ? ` --${toKebabCase(input.name)} <value>` : ` [--${toKebabCase(input.name)} <value>]`);
  return rendered.join("");
}

function formatParam(input: NonNullable<CommandDefinition["params"]>[number]): string {

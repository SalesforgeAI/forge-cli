...
function cmd(input: CommandInput): CommandDefinition {
...
function param(name: string, type: ParamDefinition["type"] = "string", required = false, description?: string): ParamDefinition {
  return {
    name,
    ...(type ? { type } : {}),
    ...(required ? { required } : {}),
...
function sfCore(method: ApiRequest["method"], path: string, options: Partial<ApiRequest> = {}): ApiRequest {
  return { product: "salesforge", salesforgeApi: "core", method, path, ...options };
}

function sfMc(method: ApiRequest["method"], path: string, options: Partial<ApiRequest> = {}): ApiRequest {
  return { product: "salesforge", salesforgeApi: "multichannel", method, path, ...options };
}

function api(product: ProductId, method: ApiRequest["method"], path: string, options: Partial<ApiRequest> = {}): ApiRequest {
  return { product, method, path, ...options };
}

function simpleProductCommands(product: ProductId): CommandDefinition[] {
  return [
...
function primeforgeCommands(): CommandDefinition[] {
...
function leadsforgeCommands(): CommandDefinition[] {
  const product: ProductId = "leadsforge";
  return [
    ...
    cmd({ name: "leadsforge_get_enrichment_results", product, group: "enrichment", subcommand: "results", description: "Get enrichment job results.", params: [param("jobID", "string", true), param("limit", "number"), param("offset", "number")], request: (a) => api(product, "GET", `/enrichment/jobs/${encStr(a.jobID)}/results`, { query: { limit: (a.limit as number | undefined) ?? 100, offset: num(a.offset) } }) }),
    cmd({ name: "leadsforge_search_lookalikes", product, group: "lookalikes", subcommand: "search", description: "Search for company lookalikes.", params: [param("domains", "array", true), param("locations", "array"), param("employeeRanges", "array"), param("fundingStages", "array"), param("categories", "array"), param("page", "number"), param("pageSize", "number")], request: (a) => api(product, "POST", "/lookalikes/search", { body: { ...a, page: (a.page as number | undefined) ?? 1, pageSize: (a.pageSize as number | undefined) ?? 25 } }) }),
...
function infraforgeCommands(): CommandDefinition[] {
  const product: ProductId = "infraforge";
  return [
...
function warmforgeCommands(): CommandDefinition[] {
...
function mailforgeCommands(): CommandDefinition[] {
...
function pick(args: ReqArgs, keys: readonly string[]): JsonObject {
...
function omit(args: ReqArgs, keys: readonly string[]): JsonObject {
...
function clean<T extends Record<string, unknown>>(object: T): JsonObject {
  const result: JsonObject = {};
...
function encStr(value: unknown): string {
  return enc(String(value));
}

function appendArrayQuery(query: JsonObject, key: string, value: unknown): void {
...
function assertEnrichment(args: ReqArgs): JsonObject {
